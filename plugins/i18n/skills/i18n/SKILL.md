...
在集成多语言字符串到代码之前，**必须先了解当前项目中相关功能的现有实现**：

1. **阅读相关代码** — 找到用户要修改的组件/页面，理解它的结构、已有的 i18n 用法和代码风格
2. **查看周边实现** — 看同模块其他组件是怎么使用 `$t()` 的，保持一致
3. **再动手集成** — 基于对现有代码的理解，选择正确的 key 和使用方式
...
2. 查看同模块中已有的 `$t()` 调用，确认命名模式和使用惯例
3. 这一步不可跳过 — 不理解上下文就集成字符串会导致风格不一致或用错 key
...
3. 如果本地找到了完全匹配的 key → **直接使用该 key，跳到第 3 步**
...
3. 替换代码中的硬编码字符串为 `$t('key_name')`，**遵循第 0 步中了解到的现有代码风格**
