...
server.tool('get-version-list', '获取指定产品的多语言版本列表。返回每个版本的 version_id、版本号、支持的平台和语言。', {
    product_id: v3_1.z.string().describe(`产品ID。常用值: ${Object.entries(PRODUCT_MAP).map(([k, v]) => `${k}=${v}`).join(', ')}`),
}, async ({ product_id }) => {
    ...
    const summary = list
        .map((v) => `- version_id=${v.version_id}, version=${v.version_number}, platforms=${v.platforms}, languages=${v.supported_languages}`)
        .join('\n');
...
server.tool('search-string', '按关键词搜索多语言字符串。可指定版本精准搜索,返回 string_id、key、中英文翻译等信息。', {
    product_id: v3_1.z.string().describe('产品ID,如 2 表示 CamScanner'),
...
server.tool('export-string', '搜索多语言字符串并导出为兼容 cs-i18n 的 locale JSON 格式。支持单语言或全部语言导出,自动将 %s 替换为 {0}/{1}/{2}。', {
    ...
    word: v3_1.z.string().describe('搜索关键词(中文或英文)'),
    version_id: v3_1.z.string().optional().describe('版本ID,不传则搜索所有版本'),
    platform_id: v3_1.z.string().optional().default('4').describe('平台ID,用于选择 key: 1=Android, 3=iOS, 4=Web'),
...
server.tool('write-locales', '搜索多语言字符串并直接写入项目的 locales 目录,兼容 cs-i18n 工具格式。自动合并到已有的 locale JSON 文件,%s 自动替换为 {0}/{1}/{2}。', {
    product_id: v3_1.z.string().describe('产品ID,如 2 表示 CamScanner'),
    word: v3_1.z.string().describe('搜索关键词(中文或英文)'),
    locales_path: v3_1.z.string().describe('locales 目录的绝对路径,如 /Users/xxx/project/src/locales'),
    version_id: v3_1.z.string().optional().describe('版本ID,不传则搜索所有版本'),
    platform_id: v3_1.z.string().optional().default('4').describe('平台ID,用于选择 key: 1=Android, 3=iOS, 4=Web'),
    ...
    for (const [langId, newEntries] of Object.entries(allStrings)) {
        ...
        fs_1.default.writeFileSync(filePath, JSON.stringify(existingObj, null, 2) + '\n');
        filesWritten++;
